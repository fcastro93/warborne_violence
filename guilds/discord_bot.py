...
class CheckPartyView(View):
    ...
    @discord.ui.button(label="All Parties", style=discord.ButtonStyle.primary, emoji="📊")
    async def all_parties_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            def get_all_parties():
                try:
                    ...
                    for party in parties:
                        ...
                        leader_name = leader.player.in_game_name if leader and leader.player else "Unknown"
                        ...
                        for member in members:
                            if member.player:
                                party_data['members'].append({
                                    'name': member.player.in_game_name,
                                    'role': member.assigned_role or member.player.game_role,
                                    'discord_id': member.player.discord_user_id,
            ...
            try:
                ...
                for party in parties_data:
                    party_overview += f"**{party['party_name']}** ({len(party['members'])}/15 members)\n"
                    party_overview += f"👑 **Leader**: {party['leader']}\n"
                    
                    # Group members by role
                    roles = {}
                    for member in party['members']:
                        if not member['is_leader']:  # Skip leader as they're shown separately
                            role = member['role'] or 'Unknown'
                            if role not in roles:
                                roles[role] = []
                            roles[role].append(member['name'])
                    ...
                    for role, members in roles.items():
                        ...
                        party_overview += f"  {role_emoji} **{role.replace('_', ' ').title()}**: {', '.join(members)}\n"
            ...
            await interaction.response.send_message(
                "❌ An error occurred while generating the parties overview.",
                ephemeral=True
            )
    
    def generate_parties_html(self, parties_data):
        """Generate HTML content for parties overview image"""
        html = """
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <h1 style="text-align: center; color: #7289da;">🎮 All Parties Overview</h1>
        ...
        for party in parties_data:
            html += f"""
                <div class="party-card">
                    <div class="party-header">{party['party_name']} ({len(party['members'])}/15)</div>
                    <div class="leader">👑 Leader: {party['leader']}</div>
            """
            ...
            for role, members in roles.items():
                ...
                html += f'<div class="member"><span class="role-emoji">{role_emoji}</span>{role.replace("_", " ").title()}: {", ".join(members)}</div>'
